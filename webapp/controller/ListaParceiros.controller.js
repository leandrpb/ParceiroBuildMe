sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.prototipoParceiros.controller.ListaParceiros", {
		...
		_onTableItemPress: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				this.doNavigate("Parceiro", oBindingContext, fnResolve, "");
		...
		_onOverflowToolbarButtonPress: function(oEvent) {
			...
			if (!oDialog) {
				oDialog = sap.ui.xmlfragment({
					fragmentName: "com.sap.build.standard.prototipoParceiros.view.ViewSettingsDialog1"
				}, this);
			...
			aCollections.push({
				id: "tabelaParceiros", //"sap_Responsive_Page_0-content-build_simple_Table-1688674541567",
				aggregation: "items"
			});
		...
		onInit: function() {
			...
			this.oRouter.getTarget("ListaParceiros").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
		...
		onExit: function() {
			...
			var aControls = [{
				"controlId": "tabelaParceiros", //"sap_Responsive_Page_0-content-build_simple_Table-1688674541567",
				"groups": ["items"]
			}, {
				"controlId": "sap_Responsive_Page_0-subHeader-build_HeaderToolbar-1688675039135-content-sap_m_ComboBox-1688675069817",
				"groups": ["items"]
			}];
		...
		_filter : function() {
			...
			this.byId("tabelaParceiros").getBinding("items").filter(oFilter, "Application");
		},
		onFilterTipoChange: function(oEvent){
			if (oEvent.getParameters().selectedItem != null) {
				var sQuery = oEvent.getParameters().selectedItem.mProperties.key;
				this._oGlobalFilter = null;

				if (sQuery) {
					this._oGlobalFilter = new Filter([
						new Filter("Tipo", FilterOperator.EQ, sQuery)
					], false);
				}
			} else {
				this._oGlobalFilter = null;
